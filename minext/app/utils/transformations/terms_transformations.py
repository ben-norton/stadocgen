...
# Rename Columns
minext_df.rename(columns={'term': 'term_local_name'}, inplace=True)
...
# Set Publication (Processing) Date
minext_df['last_published'] = formatted_date

#minext_df.sort_values(by='term_local_name', axis='index', inplace=True, na_position='last')

minext_df.loc[minext_df['class_name'] == 'MaterialAssertion', 'assertion_type_iri'] = 'http://rs.tdwg.org/geoext/assertion-types/' + minext_df['term_local_name']
...
minext_df['note'] = minext_df['note'].str.replace('"', '')
#minext_df['note'] = minext_df['note'].str.replace(r'"', '')
...
metadata_src = str(path)+'/data/sources/metadata-terms.csv'
metadata_csv = str(path)+'/data/output/metadata-termlist.csv'
shutil.copy(metadata_src, metadata_csv)
